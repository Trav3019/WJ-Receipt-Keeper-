        ...
        months.map((month) => {
          ...
          return (
            <div key={month} className="space-y-2">
              {/* Month header */}
              <div className="flex items-center justify-between bg-brand-700 text-white rounded-lg px-4 py-3">
                <div>
                ...
                <div className="flex items-center gap-6 text-sm">
                  <span>GST: <strong>{fmt(mGst)}</strong></span>
                  <span>PST: <strong>{fmt(mPst)}</strong></span>
                  <span className="text-lg font-bold">{fmt(mTotal)}</span>
                  <a
                    href={`/api/export?month=${month}`}
                    className="ml-2 bg-brand-600 hover:bg-brand-500 rounded-md px-3 py-1 text-xs font-semibold transition-colors"
                  >
                    Export
                  </a>
                </div>
